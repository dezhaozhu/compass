	...
	*/
	...
	async handleSignOut() {
		try {
			...
			vscode.window.showInformationMessage("Successfully logged out of Compass")
		} catch (error) {
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		try {
			...
			vscode.window.showErrorMessage(
				"Compass: Local webview dev server is not running, HMR will not work. Please run 'npm run dev:webview' before launching the extension to enable HMR. Using bundled assets.",
			)
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		const mcpServersDir = path.join(userDocumentsPath, "Compass", "MCP")
		try {
			await fs.mkdir(mcpServersDir, { recursive: true })
		} catch (error) {
			return "~/Documents/Compass/MCP" // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
		}
	...
	async handleAuthCallback(customToken: string, apiKey: string) {
		try {
			...
			await this.postStateToWebview()
			vscode.window.showInformationMessage("Successfully logged in to Compass")
		} catch (error) {
			console.error("Failed to handle auth callback:", error)
			vscode.window.showErrorMessage("Failed to log in to Compass")
